...
public class SpellObjectConfigurator : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        if (spell != null)
        {
            if (spell.spellType == Spell.SpellType.Single)
                ...
                {
                    var npc = myTarget.gameObject.GetComponent<NPC>();

                    if (spell.spellFlag == Spell.SpellFlag.DamagePerSecond)
                    {
                        myTarget.gameObject.GetComponent<NPC>().getHit(
                            Random.Range(spell.spellMinDamage, spell.spellMaxDamage), Player.Instance.gameObject);
            ...
            {
                var hitColliders = Physics.OverlapSphere(myTransform.position, 5.0f);

                //for each collider in that radius will take damage
                for (var i = 0; i < hitColliders.Length; i++)
                    if (hitColliders[i].tag == "NPC")
                    {
                        PhotonNetwork.Instantiate("Particles/" + spell.spellCollisionParticle.name,
                            hitColliders[i].transform.position, Quaternion.identity, 0);
                        ...
                        hitColliders[i].gameObject.GetComponent<NPC>().getHit(
                            Random.Range(spell.spellMinDamage, spell.spellMaxDamage), Player.Instance.gameObject);
                    }
    ...
    private void Update()
    {
        ...
        if (spell != null)
            if (spell.spellType == Spell.SpellType.Single)
            {
                ...
                if (spell.spellDirection == Spell.SpellDirection.Directional) MoveStraightForward();

                //Instantiated object will follow target.
                if (spell.spellDirection == Spell.SpellDirection.Follow) FollowTarget();
            }
    }
    ...
    private void OnCollisionEnter(Collision col)
    {
        ...
        if (col.gameObject.tag == "NPC")
        {
            ...
            PhotonNetwork.Instantiate("Particles/" + spell.spellCollisionParticle.name, cp.point, Quaternion.identity,
                0);
            ...
            else
            {
                if (npc)
                    ...
                    Debug.LogWarning("The DamageByFlag script not found in" + " " + col.gameObject.name +
                                     ".Please assign the DamageByFlag script.");
            }
