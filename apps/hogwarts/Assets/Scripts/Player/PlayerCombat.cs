...
public class PlayerCombat : MonoBehaviour
{
    ...
    private readonly float spellAnimTime = 1.6f;
    ...
    {
        if (Player.Instance.target == null) return;
        StartCoroutine(SpellCast(spellList[key], key));
    }

    public IEnumerator SpellCast(Spell spell, int uiPos)
    {
        if (spell.spellManaCost > Player.Instance.mana || spell.minLevel > Player.Instance.level) yield break;
    ...
    private void SpellSetUp(Spell spell)
    {
        ...
        if (spell.spellType == Spell.SpellType.Single)
        {
            ...
            {
                spellObject = PhotonNetwork.Instantiate("Particles/" + spell.spellPrefab.name,
                    transform.position + transform.up, transform.rotation, 0);
            ...
            {
                spellObject = PhotonNetwork.Instantiate("Particles/" + spell.spellPrefab.name,
                    transform.position + transform.up, Quaternion.identity, 0);
                spellObject.name = spell.spellName;
            ...
            {
                spellObject = PhotonNetwork.Instantiate("Particles/" + spell.spellPrefab.name,
                    Player.Instance.target.transform.position, Quaternion.identity, 0);
                spellObject.name = spell.spellName;
                spellObject.GetComponent<SpellObjectConfigurator>().myTarget = Player.Instance.target.transform;
            }
        }
        ...
        else if (spell.spellType == Spell.SpellType.Aoe)
        {
            if (spell.spellPosition == Spell.SpellPosition.TargetTransform)
                ...
                spellObject = PhotonNetwork.Instantiate("Particles/" + spell.spellPrefab.name, transform.position,
