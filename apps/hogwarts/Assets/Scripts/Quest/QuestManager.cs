...
public class QuestManager : MonoBehaviour
{
    ...
    private readonly Dictionary<int, List<int>> npcQuests = new();
    ...
    public void Start()
    {
        ...
        foreach (var task in Service.db.Select<Task>("FROM tasks"))
        {
    ...
    public void addQuest(Quest quest)
    {
        ...
        foreach (var task in quest.tasks.Values) task.create();
    ...
    public void completeQuest(Quest quest)
    {
        ...
        foreach (var id in quest.tasks.Keys) Service.db.Delete("tasks", id);

        // give quest's loot
        foreach (var entry in quest.loot) Player.Instance.addItem(entry.Key, entry.Value);
        ...
        foreach (var task in quest.tasks.Values)
            if (task.type == type && task.action == action &&
                ((task.type != Task.ActorType.NPC && task.id == id) ||
                 (task.type == Task.ActorType.NPC && ((task.idType == Task.IdType.Id && task.id == id) ||
                                                      (task.idType == Task.IdType.Template && task.id == extraId)))
                )
               )
            {
                if (task.quantity > 0)
    ...
    {
        if (npcQuests.ContainsKey(id)) return npcQuests[id];
        return new List<int>();
    }

    public void assignToNPC(Quest quest)
    {
        if (!npcQuests.ContainsKey(quest.assigner)) npcQuests.Add(quest.assigner, new List<int>());
