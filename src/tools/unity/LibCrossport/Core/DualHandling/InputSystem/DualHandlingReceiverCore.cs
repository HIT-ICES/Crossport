...
namespace Anonymous.Crossport.Core.DualHandling.InputSystem
{
    ...
    ///
    /// </summary>
    partial class DualHandlingReceiverCore : InputManager, IDisposable
    {
        ...
        public DualHandlingReceiverCore(RTCDataChannel channel)
        {
        ...
        ~DualHandlingReceiverCore() { this.Dispose(); }
        ...
        private void OnMessage(byte[] bytes)
        {
            ...
            // FS: DualHandling.SendEventFeedBack
            if (message.type is not InputRemoting.MessageType.NewEvents) return;
            message.type = InputRemoting.MessageType.NewEventFeedback;
            _channel.Send(MessageSerializer.Serialize(ref message));
        }
        ...
        ///
        /// </summary>
        public override IEnumerable<string> layouts => Enumerable.Empty<string>();


        /// <summary>
        ///
        /// </summary>
        public ReadOnlyArray<InputDevice> remoteDevices => new(_remoteDevices.ToArray());

        public ReadOnlyArray<string> remoteLayouts
        {
            get { return new ReadOnlyArray<string>(_remoteLayouts.Keys.ToArray()); }
        }
        ...
        public override void QueueEvent(InputEventPtr ptr)
        {
            ...
            if (device is Keyboard
             && (eventType == StateEvent.Type || eventType == DeltaStateEvent.Type || eventType == TextEvent.Type))
            {
