...
namespace Ices.Crossport.Core.DualHandling.InputSystem
{
    ...
    public sealed class InputRemoting : IObservable<InputRemoting.Message>, IObserver<InputRemoting.Message>
    {
        ...
        public struct Message
        {
            ...
            /// <inheritdoc />
            public override bool Equals(object obj) { return obj is Message other && Equals(other); }

            /// <inheritdoc />
            public readonly override int GetHashCode() { return HashCode.Combine(participantId, (int)type, data); }
        }
        ...
        void IObserver<Message>.OnNext(Message msg)
        {
            ...
            case MessageType.StopSending:
                ...
                break;
            case MessageType.NewEventFeedback:
                // Dual handling Local
                msg.type = MessageType.NewEvents;
                var time = BitConverter.ToInt64(msg.data[..((int)NewEventsMsg.TIME_STAMP_SIZE)]);
        ...
        private void SendDevice(InputDevice device)
        {
            ...
            var stateEventMessage = NewEventsMsg.CreateStateEvent(device, now);
            Send(stateEventMessage);
            EventLogger.LogSend(now);
        }
        ...
        private unsafe void SendEvent(InputEventPtr eventPtr, InputDevice device)
        {
            ...
            if (device is { remote: true })
                return;
            var now = DateTime.UtcNow;
            var message = NewEventsMsg.Create(eventPtr.data, 1, now);
            Send(message);
            EventLogger.LogSend(now);
        }
        ...
        [Flags] private enum Flags { Sending = 1 << 0, StartSendingOnConnect = 1 << 1 }
        ...
        {
            public static void Process(InputRemoting Receiver) { Receiver.StartSending(); }
        }

        private static class StopSendingMsg
        {
            public static void Process(InputRemoting Receiver) { Receiver.StopSending(); }
        }
            ...
            public static Message? Create(InputRemoting sender, string layoutName)
            {
                ...
                catch (Exception exception)
                {
                    ...
                    (
                        string.Format
                        (
                            "Could not load layout '{0}'; not sending to remote listeners (exception: {1})",
                            layoutName,
                            exception
                        )
                    );
        ...
        {
            public const uint TIME_STAMP_SIZE = 8u;
            ...
            public static unsafe Message CreateStateEvent(InputDevice device, DateTime now)
            {
                using (StateEvent.From(device, out var eventPtr))
                    return Create(eventPtr.data, 1, now);
            }

            public static unsafe Message Create(InputEvent* events, int eventCount, DateTime now)
            {
                ...
                var timeStamp = BitConverter.GetBytes(now.ToBinary());
                totalSize += TIME_STAMP_SIZE;
                ...
                fixed (byte* dataPtr = data)
                {
                    ...
                    UnsafeUtility.MemCpy(dataPtr + TIME_STAMP_SIZE, events, totalSize);
                }
            ...
            public static unsafe void Process(InputRemoting Receiver, Message msg)
            {
                ...
                fixed (byte* dataPtr = msg.data)
                {
                    ...
                    var eventCount = 0;
                    var eventPtr = new InputEventPtr((InputEvent*)(dataPtr + TIME_STAMP_SIZE));
                    var senderIndex = Receiver.FindOrCreateSenderRecord(msg.participantId);
