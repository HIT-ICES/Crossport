...
namespace Ices.Crossport.Core.DualHandling
{
    ...
    internal class InputReceiverEditor : UnityEditor.Editor
    {
        public void OnEnable()
        {
            ...
            m_Actions = serializedObject.FindProperty(DualHandlingInputReceiver.ActionsPropertyName);
            m_ActionEvents = serializedObject.FindProperty(DualHandlingInputReceiver.ActionEventsPropertyName);
            m_DefaultActionMap = serializedObject.FindProperty(DualHandlingInputReceiver.DefaultActionMapPropertyName);
        }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (m_ActionMapOptions != null && m_ActionMapOptions.Length > 0)
            {
                ...
                (
                    m_DefaultActionMapText,
                    m_SelectedDefaultActionMap,
                    m_ActionMapOptions
                );
            ...
            m_EventsGroupUnfolded = EditorGUILayout.Foldout
                (m_EventsGroupUnfolded, m_EventsGroupText, toggleOnLabelClick: true);
                                ...
                                if (m_ActionMapEventsUnfolded[n])
                                {
                                    for (var i = 0; i < m_ActionNames.Length; ++i)
                                    {
                                        ...
                                        EditorGUILayout.PropertyField
                                            (m_ActionEvents.GetArrayElementAtIndex(i), m_ActionNames[i]);
                                    }
                                }
                            }
                        }
        ...
        private void OnActionAssetChange()
        {
            ...
            var playerInput = (DualHandlingInputReceiver)target;
            var asset = (InputActionAsset)m_Actions.objectReferenceValue;
            if (asset == null)
            {
        ...
        [NonSerialized] private readonly GUIContent m_DefaultActionMapText =
            EditorGUIUtility.TrTextContent
                ("Default Map", "Action map to enable by default. If not set, no actions will be enabled by default.");

        [NonSerialized] private readonly GUIContent m_DebugText = EditorGUIUtility.TrTextContent("Debug");

        [NonSerialized] private readonly GUIContent m_DevicesGroupText =
