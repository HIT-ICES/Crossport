...
public class InputSystemAgent : MonoBehaviour
{
    ...
    [SerializeField] private DualHandlingInputReceiver _inputReceiver;

    private void Awake()
    {
        _instance = this;
        EnhancedTouchSupport.Enable();
        var actions = (_inputReceiver?.actions ?? defaultInputAsset).actionMaps[0].actions;
    ...
    where T : struct
    {
        void _handler(InputAction.CallbackContext context) { writeTo.Value = context.ReadValue<T>(); }
        return _handler;
    }

    private Action<InputAction.CallbackContext> CreateOnKeyEvent(string key)
    {
        void _handler(InputAction.CallbackContext context) { _key[key] = context.control as ButtonControl; }
        return _handler;
    }
